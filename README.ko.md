...
- 범위가 흐릴 때 먼저 적는 짧은 spec과 구현 계획
- 이슈 중심 실행 방식
- 역할별로 나뉜 `docs/` 구조
- 저장소 진입점 역할을 하는 `AGENTS.md` 패턴
- 왜 이 결정을 했는지 남기는 decision record
- PR 스레드 밖에서도 다시 볼 수 있는 review note
- 저장소별 `먼저 읽고 피해야 할 실수`를 담는 guardrails
- 무엇을 만들었는지보다 어떻게 만들었는지를 남기는 기술 학습 노트
- 실패 / 트러블슈팅 기록 루프
- 드리프트 / 죽은 코드 / 문서 정리를 위한 저장소 위생 루프
- 부트스트랩, spec 작성, planning, 이슈 워크플로, troubleshooting, decision 기록, review, 학습 노트, 서브 에이전트 운영을 위한 재사용 가능한 Codex 스킬
...
- `이 변경에 대한 짧은 spec을 먼저 쓰고, 이어서 이슈 문서도 만들어줘`
- `이 저장소에 새 이슈 문서를 만들어줘. 제목은 Reduce full rerenders`

## 핵심 플로우

1. 표준 `docs/` 하네스를 저장소에 설치합니다.
2. 범위, 경계, 성공 기준이 흐리면 먼저 짧은 spec을 작성합니다.
3. 구현을 시작하기 전에 새 이슈를 엽니다.
4. 작업을 작은 검증 단위로 나눕니다.
5. 채팅 기록만 보지 않고, 해당 이슈를 기준으로 구현합니다.
6. 위험하거나 과거에 자주 틀린 패턴을 건드리기 전에는 guardrails를 먼저 읽습니다.
7. 결정 기록, troubleshooting note, review note, learning note를 각각 맞는 위치에 남깁니다.
8. 서브 에이전트는 분석, 구현, 리뷰처럼 경계가 분명한 작업에서만 사용합니다.
9. 드리프트와 가비지 누적을 막기 위해 위생 점검 루프를 돌립니다.
...
- 구현 전에 범위와 성공 기준을 정리하는 방법
- 무엇을 먼저 읽을지
- 어떻게 이슈를 열고 추적할지
- 결정, 실패, 리뷰, 학습을 어떻게 기록할지
